    ...
    public static class TextHandler
    {
        ...
        public static string ReplaceKeyWords(string description)
        {
            ...
            return Regex.Replace(description, pattern, match =>
            {
                ...
                {
                    ErrorHandler.Warning("Unable to find Key Word : " + keyWord);
                    return keyWord;
                }
        ...
        public static string ReplaceStateEffectOverridingProperties(string text, List<SBonusStatsOverride> bonusStatsOverrides, int level)
        {
            ...
            return Regex.Replace(text, pattern, match =>
            {
                ...
                if (bonusStatsOverrides.IsNullOrEmpty() || ! bonusStatsOverrides.Any(t => t.BonusStat.StateEffectProperty.ToString() == propertyStr && (specialCondition == null || (t.BonusStat.SpecialConditions != null && t.BonusStat.SpecialConditions.Contains(specialCondition)))))
                {
                    ErrorHandler.Warning("Unable to find any stat " + propertyStr + " in bonusStatsOverrides");
                }

                // get value
                SBonusStats bonusStat = bonusStatsOverrides.Where(t => t.BonusStat.StateEffectProperty.ToString() == propertyStr && (specialCondition == null || (t.BonusStat.SpecialConditions != null && t.BonusStat.SpecialConditions.Contains(specialCondition)))).First().BonusStat;
                string value = bonusStat.Get(level, 1, specialCondition: specialCondition).ToString();

                // is float : format into clean string
                if (float.TryParse(value, out float fValue))
                    value = FormatPropertyValue(fValue, bonusStat.StateEffectProperty.ToString());
                value = FormatScaling(value.ToString(), bonusStat.ScalingDirection);
        ...
        public static string ReplaceSubStateEffects(string text, SpellData spellData)
        {
            ...
            return Regex.Replace(text, pattern, m =>
            {
                ...
                var stateEffects = side == "Ally" ? spellData.AllyStateEffects : spellData.EnemyStateEffects;
